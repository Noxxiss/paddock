...
<main>
  {#if token}
    ...
    {:else if page === 'paddocks'}
      <PaddockList onback={() => page = 'farm-settings'} farmBoundary={farm.boundary_geojson && (typeof farm.boundary_geojson === 'string' ? JSON.parse(farm.boundary_geojson) : farm.boundary_geojson)} />
